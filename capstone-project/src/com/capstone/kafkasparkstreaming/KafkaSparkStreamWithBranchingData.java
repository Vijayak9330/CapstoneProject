...
public class KafkaSparkStreamWithBranchingData {
	...
	public static void main(String[] args) {
		...
		try {
			CAStreamingQuery = CADS.writeStream().trigger(Trigger.ProcessingTime(30, TimeUnit.SECONDS)).format("csv")
					.option("checkpointLocation", "c:/capstone/ca").option("header", true)
					.start("c:/structuredstream-kafka-CA-out");
			SBStreamingQuery = SBDS.writeStream().trigger(Trigger.ProcessingTime(30, TimeUnit.SECONDS)).format("json")
					.option("checkpointLocation", "c:/capstone/sb").option("header", true)
					.start("c:/structuredstream-kafka-SB-out");

			RDStreamingQuery = RDDS.writeStream().trigger(Trigger.ProcessingTime(30, TimeUnit.SECONDS)).format("avro")
			...
			accDS.groupBy(col("branch"), col("accountType")).count().withColumnRenamed("count", "number_of_accounts")
					.writeStream().foreachBatch((batchDF, batchId) -> {
						batchDF.write().format("jdbc").option("url", "jdbc:mysql://localhost:3306/trainingdb")
								.option("dbtable", "analytics_table_tb1").option("user", "root")
